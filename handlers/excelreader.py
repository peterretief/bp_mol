...
class readexcel(object):
	...
	filekey = 'q1OncSjqs9SlN_k_3Zt1PA=='
...
#       self.__filename__ = filename
        self.__book__ = xlrd.open_workbook(file_contents=blobstore.BlobReader(filekey).read())
        self.__sheets__ = {}
        self.__sheetnames__ = []
    ...
    def __formatrow__(self, types, values, wanttupledate):
        ...
        for i in range(len(types)):
            ...
            if type == 2:
                ...
                datetuple = xlrd.xldate_as_tuple(float(value), 1) #(datetime(*(xlrd.xldate_as_tuple(sh.cell_value(a, 1), 0))[0:6])).strftime('%d-%m-%Y')
		#(datetime(value).strftime('%d-%m-%Y')
                if wanttupledate:
                    value = datetuple
                else:
