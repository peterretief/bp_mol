...
#add index document
"""
def CreateDocument(vessel_name, voyage, port, mkey):
    author = ""
    if author:
    ...
    return search.Document(
        fields=[search.TextField(name='author', value=nickname),
               ...
               search.TextField(name="key", value=mkey),
                search.DateField(name="updated", value=datetime.now().date())])

"""
"""
def NewDocument(vessel_name):
    author = ""
    if author:
        nickname = author.nickname().split('@')[0]
    else:
...
#			pass
		...
		filekey = blobstore.BlobInfo.all()[3].key()
		...
		return self.render_template("filerows.html", **params)
...
def addVesselData(wb):
        delete_all_in_index("CreateVesselData")
	...
	while curr_row < num_rows:
		...
		while curr_cell < num_cells:
			...
			#    print('%0.2f %s %s' % (cell_value, curr_row, curr_cell) )
	       		...
	       		if (cell_type == 1):
	                    search.Index(name=_INDEX_NAME).put(CreateVesselData(cell_value, int(curr_row), int(curr_cell)))
